...
def log_epoch_metrics(run_id: int, epoch: int, loss: float, learning_rate: float):
    ...
    conn.close()
    logger.info(
        f"[TrainingMetrics] Logged epoch {epoch} for run {run_id}: loss={loss:.4f}"
    )
...
def finish_training_run(run_id: int, status: str = "completed"):
    ...
    conn.close()
    logger.info(
        f"[TrainingMetrics] Finished training run {run_id} with status: {status}"
    )

...
def list_appointments(
    user_id: int, start_date: str = None, end_date: str = None, status: str = None
) -> List[dict]:
...
def update_appointment(
    ...
    with sqlite3.connect(str(PERSONAL_DB_PATH)) as conn:
        ...
        query = (
            f"UPDATE appointments SET {', '.join(updates)} WHERE id = ? AND user_id = ?"
        )
        cursor.execute(query, params)
        conn.commit()
...
def list_tasks(
    user_id: int, status: str = None, priority: str = None, category: str = None
) -> List[dict]:

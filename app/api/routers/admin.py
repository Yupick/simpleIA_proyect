...
from ...db.sqlite import list_users_with_roles, set_admin
from ...db.config_db import set_config, get_all_config
import sqlite3
...
async def admin_dashboard(
    ...
    return templates.TemplateResponse(
        "admin.html", {"request": request, "user": current_user}
    )
...
async def list_users(
    page: int = 1, limit: int = 20, current_user=Depends(get_current_admin_user)
):
    """
...
async def update_user_role(
    user_id: int, role_update: RoleUpdate, current_user=Depends(get_current_admin_user)
):
    """
...
async def update_configuration(
    config: ConfigUpdate, current_user=Depends(get_current_admin_user)
):
    """
...
async def upload_logo(
    file: UploadFile = File(...), current_user=Depends(get_current_admin_user)
):
    """
...
async def list_feedback(
    ...
    with sqlite3.connect(str(FEEDBACK_DB_PATH)) as conn:
        ...
        for row in cursor.fetchall():
            feedbacks.append({"id": row[0], "text": row[1], "timestamp": row[2]})
...
async def delete_feedback(
    feedback_id: int, current_user=Depends(get_current_admin_user)
):
    """
    Elimina un feedback.
    Requiere permisos de administrador.
...
async def get_current_provider(current_user=Depends(get_current_admin_user)):
    ...
    if config_path.exists():
        try:
            ...
            logging.getLogger(__name__).exception("Error leyendo config.json: %s", e)
    ...
    # Mapear para display
    display_mapping = {"hf": "huggingface", "claude": "claude", "openai": "openai"}
...
async def get_available_models(
    provider: Optional[str] = None, current_user=Depends(get_current_admin_user)
):
    """
    ...
    all_models = {
        "huggingface": {
            "spanish": [
                ...
                {
                    "id": "PlanTL-GOB-ES/gpt2-base-bne",
                    "name": "GPT-2 BNE (Gobierno ES) ⭐",
                    "size": "500MB",
                },
                {
                    "id": "datificate/gpt2-small-spanish",
                    "name": "GPT-2 Small Spanish",
                    "size": "500MB",
                },
                {
                    "id": "flax-community/gpt-2-spanish",
                    "name": "GPT-2 Spanish (Flax)",
                    "size": "500MB",
                },
            ],
            "english": [
    ...
    if provider:
        return {"provider": provider, "models": all_models.get(provider, [])}
...
async def switch_provider(
    data: ProviderSwitch, current_user=Depends(get_current_admin_user)
):
    """
    ...
    if data.provider == "openai":
        ...
        if not data.api_key and not has_saved_key:
            raise HTTPException(
                status_code=400, detail="La API key de OpenAI es requerida"
            )
    ...
    # Normalizar provider a formato interno
    provider_internal = {"huggingface": "hf", "claude": "claude", "openai": "openai"}
...
async def get_stats(current_user=Depends(get_current_admin_user)):
    ...
    stats["db_sizes"] = {
        ...
        "feedback_db": (
            os.path.getsize(FEEDBACK_DB_PATH) if FEEDBACK_DB_PATH.exists() else 0
        ),
    }

    ...
    try:
        ...
        if not user_id:
            return WhatsAppResponse(
                success=False, error="Usuario no registrado. Contacte con soporte."
            )

        # Verificar que el usuario existe
        user = get_user_by_id(user_id)
        if not user:
            return WhatsAppResponse(success=False, error="Usuario no encontrado")
        ...
        return WhatsAppResponse(success=True, response=response_text)

    except Exception as e:
        logger.error(f"Error processing WhatsApp message: {e}")
        return WhatsAppResponse(success=False, error="Error al procesar el mensaje")
...
async def link_phone_to_user(phone_number: str, user_id: int):
    ...
    return {
        "success": True,
        "message": f"Teléfono {phone_number} vinculado a user {user_id}",
    }

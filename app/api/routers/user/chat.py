...
async def send_message(
    message: MessageCreate, current_user: dict = Depends(get_current_regular_user)
):
    ...
    if message.assistant_type == "commercial":
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Tipo de asistente inválido"
        )
...
async def list_conversations(
    ...
    conversations = conv_db.list_conversations(
        user_id=current_user["id"], assistant_type=assistant_type
    )
...
async def get_conversation(
    conversation_id: int, current_user: dict = Depends(get_current_regular_user)
):
...
async def get_activity(
    days: int = 7, current_user: dict = Depends(get_current_regular_user)
):

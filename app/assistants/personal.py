...
"""
    ...
    def get_pending_tasks_by_priority(self) -> Dict[str, List[Dict]]:
        ...
        tasks = personal_db.list_tasks(user_id=self.user_id, status="pending")
    ...
    def get_overdue_tasks(self) -> List[Dict]:
        ...
        all_tasks = personal_db.list_tasks(user_id=self.user_id, status="pending")
    ...
    async def process_message(
        ...
        if any(
            ...
            for word in ["cita", "reunión", "reuniones", "agenda", "calendario"]
        ):
        ...
        if any(
            word in message_lower
            for word in ["tarea", "tareas", "pendiente", "hacer", "completar"]
        ):
            ...
            if overdue:
                ...
                for task in overdue:
                    additional_context += (
                        f"- {task['title']} (Venció: {task['due_date']})\n"
                    )

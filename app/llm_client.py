...
async def get_system_info():
    ...
    try:
        ...
        if config_path.exists():
            try:
                with open(config_path, "r") as f:
                    ...
                    model = config.get(
                        "model_name", config.get("selected_model", model)
                    )
        ...
        return JSONResponse(
            content={"provider": "Error", "model": str(e)}, status_code=500
        )
...
async def index(request: Request):
    ...
    return templates.TemplateResponse(
        "index.html", {"request": request, "user": user, "messages": []}
    )
    ...
    try:
        ...
        if payload.get("stream"):
            return StreamingResponse(
                response.iter_content(chunk_size=1024), media_type="text/event-stream"
            )
...
async def feedback_proxy(request: Request):
    ...
    try:
        ...
        response = requests.post(
            f"{API_URL}/feedback", json={"text": text}, headers=headers
        )
        response.raise_for_status()
...
async def admin_dashboard_main(request: Request):
    ...
    return templates.TemplateResponse(
        "admin.html", {"request": request, "user": user, "active_tab": "dashboard"}
    )
...
async def user_commercial_products(request: Request):
    ...
    return templates.TemplateResponse(
        "user/commercial/products.html", {"request": request, "user": user}
    )
...
async def user_personal_calendar(request: Request):
    ...
    return templates.TemplateResponse(
        "user/personal/calendar.html", {"request": request, "user": user}
    )
...
async def user_personal_tasks(request: Request):
    ...
    return templates.TemplateResponse(
        "user/personal/tasks.html", {"request": request, "user": user}
    )
...
async def user_chat(request: Request):
    ...
    return templates.TemplateResponse(
        "user/chat.html", {"request": request, "user": user}
    )
...
async def admin_users_page(request: Request):
    ...
    if not user or not user.get("is_admin"):
        return RedirectResponse(url="/login", status_code=302)
    return templates.TemplateResponse(
        "admin_users.html", {"request": request, "user": user, "active_tab": "users"}
    )
...
async def admin_config_page(request: Request):
    ...
    if not user or not user.get("is_admin"):
        return RedirectResponse(url="/login", status_code=302)
    return templates.TemplateResponse(
        "admin_config.html", {"request": request, "user": user, "active_tab": "config"}
    )
...
async def admin_feedback_page(request: Request):
    ...
    return templates.TemplateResponse(
        "admin_feedback.html",
        {"request": request, "user": user, "active_tab": "feedback"},
    )
...
async def admin_providers_page(request: Request):
    ...
    return templates.TemplateResponse(
        ...
        {"request": request, "user": user, "active_tab": "providers"},
    )
...
async def admin_stats_page(request: Request):
    ...
    if not user or not user.get("is_admin"):
        return RedirectResponse(url="/login", status_code=302)
    return templates.TemplateResponse(
        "admin_stats.html", {"request": request, "user": user, "active_tab": "stats"}
    )
...
async def admin_training_page(request: Request):
    ...
    return templates.TemplateResponse(
        "admin_training.html",
        {"request": request, "user": user, "active_tab": "training"},
    )
...
async def admin_users_proxy(request: Request, page: int = 1, limit: int = 20):
    ...
    try:
        ...
        response = requests.get(
            f"{API_URL}/admin/users?page={page}&limit={limit}", headers=headers
        )
        response.raise_for_status()
        return JSONResponse(content=response.json())
    except Exception as e:
...
@app.api_route(
    "/api/admin/users/{user_id}/role", methods=["POST"], response_class=JSONResponse
)
async def admin_user_role_proxy(request: Request, user_id: int):
    ...
    try:
        ...
        headers = {"Authorization": f"Bearer {token}"}
        response = requests.post(
            f"{API_URL}/admin/users/{user_id}/role", json=body, headers=headers
        )
        response.raise_for_status()
        return JSONResponse(content=response.json())
    except Exception as e:
...
@app.api_route(
    "/api/admin/users/{user_id}", methods=["DELETE"], response_class=JSONResponse
)
...
@app.api_route(
    "/api/admin/feedback/{feedback_id}", methods=["DELETE"], response_class=JSONResponse
)
async def admin_feedback_delete_proxy(request: Request, feedback_id: int):
    ...
    try:
        ...
        response = requests.delete(
            f"{API_URL}/admin/feedback/{feedback_id}", headers=headers
        )
        response.raise_for_status()
        return JSONResponse(content=response.json())
    except Exception as e:
...
@app.api_route(
    "/api/admin/config", methods=["GET", "POST"], response_class=JSONResponse
)
async def admin_config_api_proxy(request: Request):
    ...
    try:
        ...
        if request.method == "GET":
            ...
            body = await request.json()
            response = requests.post(
                f"{API_URL}/admin/config", json=body, headers=headers
            )
...
async def admin_config_logo_proxy(request: Request):
    ...
    try:
        ...
        response = requests.post(
            f"{API_URL}/admin/config/logo", files=files, headers=headers
        )
        response.raise_for_status()
        return JSONResponse(content=response.json())
    except Exception as e:
...
async def admin_providers_switch_proxy(request: Request):
    ...
    try:
        ...
        response = requests.post(
            f"{API_URL}/admin/providers/switch", json=body, headers=headers
        )
        response.raise_for_status()
        return JSONResponse(content=response.json())
    except requests.exceptions.RequestException as e:
        ...
        return JSONResponse(
            content={"error": error_msg, "detail": error_msg}, status_code=500
        )
    except Exception as e:
        logger.exception("Error inesperado en providers/switch proxy")
        return JSONResponse(
            content={"error": str(e), "detail": str(e)}, status_code=500
        )
...
async def training_proxy(request: Request, path: str):
    ...
    try:
        ...
        if request.method == "GET":
            ...
            return JSONResponse(
                content={"error": "Method not allowed"}, status_code=405
            )
        ...
        return JSONResponse(
            content={"error": error_msg, "detail": error_msg}, status_code=status_code
        )
    except Exception as e:
...
from app.api.routers.user.chat import router as chat_api_router  # noqa: E402
from app.api.routers.user.products import router as products_api_router  # noqa: E402
from app.api.routers.user.personal import router as personal_api_router  # noqa: E402
...
async def get_user_dashboard(request: Request):
    ...
    try:
        ...
        appointments = personal_db.list_appointments(
            user_id=user_id, start_date=today, end_date=next_week, status="scheduled"
        )
        ...
        upcoming_appointments = sorted(appointments, key=lambda x: x["start_datetime"])[
            :3
        ]
...
async def get_user_analytics(request: Request, days: int = 7):
    ...
    try:
        ...
        appointments = personal_db.list_appointments(
            user_id=user_id, start_date=today, end_date=end_date
        )
        ...
        return JSONResponse(
            content={
                ...
                "recent_activity": generate_recent_activity(
                    all_tasks, appointments, products
                ),
            }
        )
    except Exception as e:
...
async def get_whatsapp_status(request: Request):
    ...
    return JSONResponse(
        content={"connected": False, "phone_number": None, "connected_since": None}
    )
...
async def generate_whatsapp_qr(request: Request):
    ...
    return JSONResponse(
        content={"success": True, "qr_code": "data:image/png;base64,..."}
    )
...
def generate_appointments_chart_data(appointments):
    ...
    return {"labels": list(weeks.keys()), "values": list(weeks.values())}
...
def calculate_assistant_usage(conversations):
    ...
    return {"personal": personal_count, "commercial": commercial_count}
...
def generate_recent_activity(tasks, appointments, products):
    ...
    # Citas recientes
    for apt in sorted(
        appointments, key=lambda x: x.get("created_at", ""), reverse=True
    )[:2]:

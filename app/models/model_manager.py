        ...
        try:
            if provider == "hf":
                ...
                _tokenizer = AutoTokenizer.from_pretrained(
                    model_to_load, clean_up_tokenization_spaces=True
                )
                ...
                _provider_instance = HuggingFaceProvider(
                    model_name=model_to_load, model=_model, tokenizer=_tokenizer
                )
            elif provider == "claude":
                api_key = api_keys_config.get("anthropic_api_key") or os.getenv(
                    "ANTHROPIC_API_KEY"
                )
                if not api_key:
                    logger.error(
                        "[Claude] API key no encontrada en config.json ni en variables de entorno"
                    )
                    raise ValueError("Claude API key is required")

                logger.info(
                    f"[Claude] Initializing Claude provider with model {model_name}"
                )
                ...
                api_key = api_keys_config.get("openai_api_key") or os.getenv(
                    "OPENAI_API_KEY"
                )
                if not api_key:
                    logger.error(
                        "[OpenAI] API key no encontrada en config.json ni en variables de entorno"
                    )
                ...
                logger.info(
                    f"[OpenAI] Initializing OpenAI provider with model {model_name}"
                )
                ...
                logger.warning(
                    f"[Unknown provider:{provider}] Falling back to HuggingFaceProvider"
                )
...
async def generate(
    ...
    if _provider_instance is not None:
        try:
            return await _provider_instance.generate(
                prompt, max_length, num_return_sequences, temperature
            )
        except Exception as e:

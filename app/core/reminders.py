...
from app.db.personal import (
    list_appointments,
    list_tasks,
    init_personal_db,
)  # noqa: E402
from app.db.sqlite import list_users_with_roles, init_user_db  # noqa: E402
...
class ReminderScheduler:
                ...
                for apt in appointments:
                    ...
                    try:
                        start_time = datetime.strptime(
                            apt["start_datetime"], "%Y-%m-%d %H:%M:%S"
                        )
                    except ValueError:
                        try:
                            start_time = datetime.strptime(
                                apt["start_datetime"], "%Y-%m-%d %H:%M"
                            )
                        except ValueError:
                            continue

                    reminder_time = start_time - timedelta(
                        minutes=apt["reminder_minutes"]
                    )
                    ...
                    if (
                        now >= reminder_time
                        and now < start_time
                        and reminder_key not in self.sent_reminders
                    ):
                        # Enviar recordatorio
                        ...
                        logger.info(
                            f"Sent appointment reminder for user {user_id}: {apt['title']}"
                        )
    ...
    async def check_task_reminders(self):
        ...
        try:
            ...
            for user in users:
                ...
                tasks = list_tasks(user_id=user_id, status="pending")
                ...
                for task in tasks:
                    ...
                    reminder_time = due_datetime - timedelta(
                        minutes=task.get("reminder_minutes", 60)
                    )
                    ...
                    if (
                        ...
                        self.sent_reminders.add(reminder_key)
                        logger.info(
                            f"Sent task reminder for user {user_id}: {task['title']}"
                        )
    ...
    async def send_reminder(
        self, user_id: int, reminder_type: str, title: str, details: str, **kwargs
    ):
        """
    ...
    async def run(self):
        ...
        self.running = True
        logger.info(
            f"Reminder scheduler started (checking every {self.check_interval}s)"
        )

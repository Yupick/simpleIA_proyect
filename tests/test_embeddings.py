...
class TestEmbeddingEndpoints:
    ...
    def auth_headers(self, client):
        ...
        # Registrar y login
        client.post(
            "/auth/register", json={"username": "embeduser", "password": "embedpass123"}
        )

        response = client.post(
            "/auth/login", data={"username": "embeduser", "password": "embedpass123"}
        )
    ...
    def test_encode_single_text(self, client, auth_headers):
        ...
        response = client.post(
            "/embed/encode", json={"texts": ["Test text"]}, headers=auth_headers
        )
    ...
    def test_add_documents(self, client, auth_headers):
        ...
        response = client.post(
            "/embed/add", json={"documents": ["Doc 1", "Doc 2"]}, headers=auth_headers
        )
    ...
    def test_search_documents(self, client, auth_headers):
        ...
        # Buscar
        response = client.post(
            "/embed/search", json={"query": "Python code", "k": 2}, headers=auth_headers
        )
    ...
    def test_search_empty_index(self, client, auth_headers):
        ...
        response = client.post(
            "/embed/search", json={"query": "test", "k": 5}, headers=auth_headers
        )
    ...
    def test_embeddings_require_auth(self, client):
        ...
        response = client.post("/embed/encode", json={"texts": ["Test"]})
    ...
    def test_encode_empty_texts(self, client, auth_headers):
        ...
        response = client.post(
            "/embed/encode", json={"texts": []}, headers=auth_headers
        )
    ...
    def test_add_empty_documents(self, client, auth_headers):
        ...
        response = client.post(
            "/embed/add", json={"documents": []}, headers=auth_headers
        )
